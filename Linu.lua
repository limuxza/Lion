...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        local TabAccentBar = Linux.Instance("Frame", {
            Parent = TabBtn,
            BackgroundColor3 = Color3.fromRGB(138, 43, 226), -- Cor de destaque (roxo)
            BackgroundTransparency = 0,
            BorderSizePixel = 0,
            Size = UDim2.new(0, 0, 0, 24), -- Largura inicial 0, altura 24
            Position = UDim2.new(0, 0, 0.5, -12), -- Posição à esquerda, centralizado verticalmente
            ZIndex = 3 -- Garante que esteja visível sobre o botão
        })

        Linux.Instance("UICorner", {
            Parent = TabAccentBar,
            CornerRadius = UDim.new(0, 4) -- Arredondamento
        })
        ...
        local function SelectTab()
            ...
            if CurrentTab and Tabs[CurrentTab] then
                ...
                -- Encolhe a barra de destaque da aba antiga
            ...
            -- Expande a barra de destaque da nova aba
        ...
        Tabs[tabIndex] = {
            ...
            TabAccentBar = TabAccentBar -- Adicione esta linha
        }                
        ...
        function TabElements.Dropdown(config)
            -- Dentro da função TabElements.Dropdown(config)

            -- Adicione estas definições de tweenInfo logo abaixo da declaração da função TabElements.Dropdown(config)
            -- ou em um escopo superior se já não existirem.
            local tweenInfoShrink = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tweenInfoExpand = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            ...
            local function PopulateOptions()
                ...
                if IsOpen then
                    for i, opt in pairs(Options) do
                        ...
                        -- Adicione a barrinha de destaque para a opção
                        local OptionAccentBar = Linux.Instance("Frame", {
                            Parent = OptBtn,
                            BackgroundColor3 = Color3.fromRGB(138, 43, 226), -- Cor de destaque (roxo)
                            BackgroundTransparency = 0,
                            BorderSizePixel = 0,
                            Size = isSelected and UDim2.new(0, 4, 0, 24) or UDim2.new(0, 0, 0, 24), -- Tamanho inicial baseado na seleção
                            Position = UDim2.new(0, 0, 0.5, -12), -- Posição à esquerda, centralizado verticalmente
                            ZIndex = 3 -- Garante que esteja visível sobre o botão
                        })

                        Linux.Instance("UICorner", {
                            Parent = OptionAccentBar,
                            CornerRadius = UDim.new(0, 4) -- Arredondamento
                        })

                        -- Armazene a referência à barrinha no botão para fácil acesso
                        OptBtn.OptionAccentBar = OptionAccentBar
                        ...
                        -- Modifique a lógica dentro de OptBtn.MouseButton1Click:
                        -- Onde você tem:
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if IsMulti then
                                ...
                                if index then
                                    table.remove(SelectedValues, index)
                                    OptBtn.TextColor3 = Color3.fromRGB(200, 200, 210)
                                    -- Encolhe a barrinha ao desmarcar
                                    TweenService:Create(OptBtn.OptionAccentBar, tweenInfoShrink, {Size = UDim2.new(0, 0, 0, 24)}):Play()
                                else
                                    table.insert(SelectedValues, opt)
                                    OptBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                                    -- Expande a barrinha ao marcar
                                    TweenService:Create(OptBtn.OptionAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 24)}):Play()
                                end
                                UpdateSelectedText()
                                spawn(function() Linux:SafeCallback(config.Callback, SelectedValues) end)
                            else
                                if opt ~= SelectedValue then
                                    -- Encolhe a barrinha da opção anteriormente selecionada
                                    for _, btn in pairs(OptionsHolder:GetChildren()) do
                                        if btn:IsA("TextButton") and btn.Text == tostring(SelectedValue) and btn.OptionAccentBar then
                                            TweenService:Create(btn.OptionAccentBar, tweenInfoShrink, {Size = UDim2.new(0, 0, 0, 24)}):Play()
                                            break
                                        end
                                    end
                                    ...
                                    for _, btn in pairs(OptionsHolder:GetChildren()) do
                                        if btn:IsA("TextButton") then
                                            if btn.Text == tostring(opt) then
                                                btn.TextColor3 = Color3.fromRGB(255, 255, 255)
                                                -- Expande a barrinha da nova opção selecionada
                                                if btn.OptionAccentBar then
                                                    TweenService:Create(btn.OptionAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 24)}):Play()
                                                end
                                            else
                                                btn.TextColor3 = Color3.fromRGB(200, 200, 210)
                                            end
            ...
            -- Modifique a função SetValue(newValue) para atualizar as barrinhas ao carregar configurações
            -- Onde você tem:
            ...
            local function SetValue(newValue)
                if IsMulti then
                    ...
                    if table.find(Options, newValue) then
                        ...
                        Selected.Text = FormatDisplayText(newValue)
                        Selected.TextColor3 = Color3.fromRGB(255, 255, 255)
                    end
                end

                -- Atualiza as barrinhas de destaque para todas as opções
                for _, btn in pairs(OptionsHolder:GetChildren()) do
                    if btn:IsA("TextButton") and btn.OptionAccentBar then
                        local isSelected = IsMulti and table.find(SelectedValues, btn.Text) or (not IsMulti and btn.Text == tostring(SelectedValue))
                        if isSelected then
                            TweenService:Create(btn.OptionAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 24)}):Play()
                            btn.TextColor3 = Color3.fromRGB(255, 255, 255)
                        else
                ...
                IsOpen = false
                UpdateDropSize()
                -- Não chame PopulateOptions() aqui, pois já atualizamos as barrinhas acima
                -- PopulateOptions() 
    ...
    task.spawn(function()
        ...
        if DefaultTab and Tabs[DefaultTab] then
            for _, tab in pairs(Tabs) do
                ...
                -- Garante que as barras de destaque das abas não selecionadas estejam encolhidas
            ...
            -- Expande a barra de destaque da aba padrão
        ...
        elseif tabOrder > 0 then
            ...
            if firstTab then
                ...
                -- Expande a barra de destaque da primeira aba
